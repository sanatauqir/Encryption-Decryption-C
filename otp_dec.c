...
void concatFiles(char* buffer, char* wholeMessage, char* cipherName, char*keyName);
...
int main(int argc, char *argv[])
{
	...
	struct hostent* serverHostInfo;
    	struct stat keySize, cipherSize;
	...
	stat(argv[2], &keySize);
	stat(argv[1], &cipherSize);	
	if (keySize.st_size < cipherSize.st_size){
		fprintf(stderr, "key is too short\n");
		exit(1);
	}

	int size= keySize.st_size + cipherSize.st_size + 10;
	...
	// Send the filetype, ciphertext file and keyfile to the server
...
void concatFiles(char* buffer, char* wholeMessage, char* cipherName, char* keyName){
	...
	char* oneChar;
	struct stat keySize, cipherSize;
	stat(cipherName, &cipherSize);
	char fileBuffer[cipherSize.st_size];
	memset(wholeMessage, '\0', sizeof(wholeMessage));
	memset(fileBuffer, '\0', sizeof(fileBuffer));

	//declare yourself
	strcat(wholeMessage, "DEC#");

	int file_descriptor = open(cipherName, O_RDONLY,0666);
